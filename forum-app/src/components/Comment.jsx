...
function Comment ({ name, id, threadId, comment, avatar, createdAt, likes, dislikes, userLoggedIn }) {
  ...
  const upVoteHandle = async () => {
    ...
    if (!hasVote) {
      const { error, message, data } = await likeComment({ threadId, commentId: id })
      if (error) {
        return showToast(`Gagal up vote comment! ${message}`, 'white', 'red')
      }
      dispatch(actionUpVoteComment(data))
    } else {
      const { error, message, data } = await neutralLikeComment({ threadId, commentId: id })
      if (error) {
  ...
  const downVoteHandle = async () => {
    const hasVote = likes.some((data) => userLoggedIn.id === data) || dislikes.some((data) => userLoggedIn.id === data)
    if (!hasVote) {
      const { error, message, data } = await dislikeComment({ threadId, commentId: id })
      if (error) {
